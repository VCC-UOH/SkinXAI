...
IMG_SIZE = (224, 224) 
BATCH_SIZE = 8  # Maintained at 8 for VRAM stability
NUM_CLASSES = 3
INITIAL_LR = 1e-4
FINE_TUNE_LR = 1e-6
PHASE1_EPOCHS = 20
...
# ==========================================
def preprocess(x, y):
    # ConvNeXt uses its own specific normalization (standardization)
    return tf.keras.applications.convnext.preprocess_input(x), y
...
model = models.Sequential([
    ...
    layers.Dropout(0.5, name="Dropout_for_UQ"), # Essential for your MC Dropout paper
...
csv_log = callbacks.CSVLogger(os.path.join(LOGS_BASE_PATH, f"log_convnext_{TIMESTAMP}.csv"), append=True)

checkpoint = callbacks.ModelCheckpoint(
    os.path.join(MODEL_SAVE_PATH, f"best_convnext_{TIMESTAMP}.keras"),
...
print(f"\n🚀 PHASE 1: Training ConvNeXt-Tiny Head for {PHASE1_EPOCHS} epochs...")
...
# ==========================================
print("\n🔓 PHASE 2: Unfreezing Final Stages of ConvNeXt...")
base_model.trainable = True

# ConvNeXt-Tiny has roughly 186 layers. 
# We unfreeze the final blocks (from layer 140) to preserve low-level edge detection.
...
# 7. FINAL EXPORT
# ==========================================
final_model_name = os.path.join(MODEL_SAVE_PATH, f"convnext_final_{TIMESTAMP}.keras")
